...
export default defineEventHandler(async (event) => {
    ...
    switch (method) {
        ...
        case 'POST': {
            ...
            const response = await databases.productos.insert(body);
            return response;
        }
        ...
        case 'PUT': {
            ...
            return await databases.productos.insert(body);
        }

        case 'DELETE': {
            const query = getQuery(event);
            if (!query._id || !query._rev) throw new Error('Se requiere _id y _rev para eliminar');
            return await databases.productos.destroy(query._id as string, query._rev as string);
        }
